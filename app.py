            ...
            if 'changes' in entry and isinstance(entry['changes'], list):
                for change in entry['changes']:
                    if change.get('field') == 'feed' and 'value' in change:
                        ...
                        if comment_data['item'] == 'comment':
                            process_facebook_comment(comment_data)

    # Check for Instagram events
...
def process_facebook_comment(comment_data):
    ...
    Parameters:
    comment_data (dict): The data of the Facebook comment.
    """
    if comment_data['item'] == 'comment':
...
def process_instagram_comment(comment_data):
    ...
    Parameters:
    comment_data (dict): The data of the Instagram comment.
    """
...
def handle_comment(comment_data):
    ...
    """
    comment_id = comment_data.get('comment_id') or comment_data['id']
...
def remove(comment_id):
    ...
    comments_collection.update_one({'id': comment_id}, {'$set': {'status': 'to_remove'}})  # Let database know, that we will soon remove the comment from the social media platform
...
def remove_comment(comment_id):
    ...
    """
    # Construct the API endpoint URL for both Facebook and Instagram
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            comments_collection.update_one({'id': comment_id}, {'$set': {'status': 'removed'}})
        else:
            print(f"Failed to remove comment with ID {comment_id}. Status code: {response.status_code}, Response: {response.text}")

    except requests.RequestException as e:
        print(f"An error occurred while trying to remove comment with ID {comment_id}: {e}")
